...
> 2026-09-10 四度精简：删除论证过程与重复条目，只留结论。

## 项目与开发环境
- Pixbian：WinUI 3 本地相册浏览器。WASDK 2.4.0 元包（WinUI 实为 2.3.6）+ `net8.0-windows10.0.26100.0`（最低 17763）。测试基线 234。
...
- Python 3.14 + Pillow 12 可用（无 numpy / ImageMagick / SVG 光栅化库）。**Pillow 12 已移除 `ImageChops.divide`** → 含 alpha 缩放改用「预乘 → Lanczos → 逐像素反预乘」。
- PowerShell 5.1 三坑：①`New-Object Type (a, b)` 被解析成数组参数 → 一律 `[Type]::new()`；②XML 适配器把无属性纯文本元素投影为 `String`（`.InnerText` 空）→ 按 `-is [XmlNode]` 分别取值；③`@($null).Count -eq 1`，判空要过滤 `$null`。
...
- **FFmpegInteropX 只被 UI 项目引用** → 复用解码策略的工厂只能放 UI 层。
- **页面需要主窗口时经 `App.Services` 按需解析**，不要注入（窗口持有页面 → 循环依赖）。
- **媒体查看三条平行链路**：双击图片 → ImageViewerWindow；双击视频 → 主窗口播放态（VideoPlayerPage 装进 `VideoHost`）；幻灯片 → SlideShowWindow。无边框全屏宿主能力在 `FullscreenWindowBase`（全屏↔窗口双形态，样式位须对称增删）。
- **轻量预览模式（已实测）**：图片文件激活且主窗口尚未创建时，App 直接开 `ImageViewerWindow` 而不建主窗口，查看器关闭即 `Environment.Exit(0)`（否则进程常驻，之后从开始菜单启动会被单实例重定向到无窗口实例）。视频仍必须走主窗口（播放器页依赖 `VideoHost` 与标题栏 Passthrough，无独立宿主）。轻量实例收到「视频文件激活 / 普通启动」须先建主窗口再分发，否则表现为「点了没反应」。
...
- 「慢」还是「冻结」判别：① 单核 100% + 日志停滞 = 布局死循环；② CPU 高 + 日志增长 = 业务慢；③ CPU 增量 0 + 全线程 Wait = 渲染停摆。死循环时托管栈为空、`crash.log` 常无痕。取证：`dotnet-stack report` 判 UI 死活；TICK 心跳间隙判同步阻塞；diag.log 判管线进度。多嫌疑用叠加减法实验逐轮排除。
- 概率性缺陷被性能优化引爆是常态，不要回滚优化，去找被掩盖的根因。

## 虚拟化：ItemsRepeater（已实测，勿再试错）
- **复用残留根治方案只有一条：让数据走 CollectionChanged（集合实例不变、原地 `Clear()` + 逐条 `Add()`）**。代价：切目录 1 次通知变 1+N 次，无感。
...
- 唯一公开扩展点是 `ItemsRepeater` + `VirtualizingLayout`；`IScrollInfo` 未公开；本版本无 `SelectionModel` → 换 ItemsRepeater 须自建选择服务。GridView **不能**外包 ScrollViewer；ItemsRepeater **必须**外包。
...
- **ScrollViewer 默认 `IsTabStop=False`，`Focus(Programmatic)` 静默失败** → 交还焦点前先设 True。
- **`FocusManager.GetFocusedElement()` 在键处理栈内返回 null**，判焦点须订阅 `FocusManager.GotFocus` 记录最近获焦元素（随 Loaded/Unloaded 订退）。
...
- `StaticResource` 引用不存在资源启动即崩；无法解析 ThemeDictionaries 内资源 → 业务画刷一律 `ThemeResource`。
- **元素级 `Resources` 里禁止放 `{ThemeResource}`**（尤其 DataTemplate）：布局 pass 内 realize 时解析会 fail-fast。配色覆盖一律放**页面级 ThemeDictionaries**（Default + Dark 各一份 hex）。
...
- **无堆栈崩溃（0xc000027b）定位**：`Start-Process` + `Get-Process` 判存活（15s）+ timing.log 最后探针，每轮 ≤1 分钟消融实验；或 `git stash` 跑 HEAD 对照 + 逐块回退二分。构建后等 30s 再启动。判活优先级：进程存活 > `overlay-hidden` 探针 > 截屏。已知诱因：模板元素挂 `PointerEntered/Exited`、模板内 x:Bind 到悬停派生属性、运行期改元素 `Visibility`。安全做法：挂冒泡 `PointerMoved` + `GetParent` 上溯、改动全部 `TryEnqueue`、只写渲染属性。
...
- **NavigationView 动态子项**：子项扁平进同一列表，只在父项 `IsExpanded` **值变化**时重算 → 动态填充后须先 false 再 true。
- **NavigationView 展开交互**：①点行与点箭头都切换展开；②点箭头不抛 `ItemInvoked`；③切换与 `ItemInvoked` 先后不定；④**在 `ItemInvoked`/`Expanding`/`Collapsed` 回调里同步改 `IsExpanded` 会 fail-fast** → 改写一律 `TryEnqueue`；⑤区分点箭头/点行只能按 `PointerPressed` 落点（`AddHandler(handledEventsToo: true)`，箭头在行右端约 44px）；⑥`Expanding`/`Collapsed` 是 NavigationView 级事件，挂 Item 上报 WMC0011。

## 控件与布局、动画
- 分组 ListViewBase 配自定义 ItemsPanel 时排列 `GroupItem`；无内置 Justified 布局与 GridLength 动画；自定义标题栏用 `InputNonClientPointerSource` Passthrough（矩形为物理像素须乘 `RasterizationScale`，布局/激活变化后重注册）。
...
- `MenuFlyout` 从 `Application.Current.Resources` 取是共享单例，重复 `ShowAt` 抛 `E_INVALIDARG` → 用工厂方法每次 `new`。`CommandBar` 动态溢出有未修 bug（issue #6450）。
...
- 符号码点：空心文件夹 `\uED25` / 实心 `\uE8B7`；线星 `\uE734` / 实心 `\uE735`；空心爱心 `\uEB51` / 实心 `\uEB52`；鼠标 `\uE962`、照片 `\uE8B9`、音乐 `\uE189`。查码点用 PowerShell + WPF 离屏渲染 PNG 目检。
- `Expander` 嵌卡片须在 `.Resources` 把三个 `Expander*BorderBrush` 与两个 BorderThickness 归零；`ToggleSwitch` 默认 `MinWidth=154px`；`Border` 只能一个 `Child`；XAML 注释内不得出现连续 `--`；`NumberBox` 清空时 `Value` 为 `NaN`。
- Storyboard 优先代码后置现场创建并以元素对象为目标（XAML TargetName 解析失败即静默无动画）；`FillBehavior` 默认 `HoldEnd` → 每轮前复位起始值。数据驱动动画须防重复触发。「控件自动隐藏」一律用 `DispatcherQueue.CreateTimer()`，绝不订阅 `CompositionTarget.Rendering`。
...
- WinUI 3 XAML 无 `EventTrigger`/`BeginStoryboard` → 状态驱动动画选「模板化控件 + VisualState」；`RepeatBehavior=Forever` 回收后不自停，`Unloaded` 里回静态态。
...
- 容器级圆角须归零（`NavigationViewContentGridCornerRadius`）；Pane 与内容区竖线来自 `ContentGrid.BorderThickness.Left`；左栏圆角来自模板 `RightCornerRadiusFilterConverter`，元素级覆盖无效。
- `PixbianContentCard*` 两刷子在 ThemeDictionaries；不随主题的画刷放主题字典之外。NavigationView 纵向：Row0 `ContentTopPadding` → Row1 `HeaderContent`（`AlwaysShowHeader="False"` 可消掉）→ Row2 `ContentPresenter`。

## 视频播放（播放器页 + 短片页）
- 页面为 DI 单例；`MediaPlayer` 惰性创建（构造期创建会 `0xC000027B`），`Unloaded` 必须释放。
- 播放态是窗口级独立分支：`PlayerRoot`（`RowSpan=2`）覆盖标题栏与内容行，切换收口在 `MainWindow.ApplyViewerChrome`；图片走独立窗口。
- 装载必须先让容器可见再赋 Content（往 Collapsed 容器塞内容不触发 Loaded）；卸载必须置空 Content。
...
- **FFmpegInteropX 要点**：`CreateFromStreamAsync` → `CreateMediaPlaybackItem()`，失败回退系统解码。硬约束：①`FFmpegMediaSource` 必须字段强引用（GC 回收中断播放）；②项目必须有 RID（`win-x64`）否则 native dll 不复制 → 静默回退；③RID 使产物落 `...\win-x64\`，`Pixbian.ps1` 硬编码路径须同步；④需 `CsWinRTWindowsMetadata` 指向本机已装 SDK（19041），新报 `CsWinRT1028` 可豁免。
- 默认 `VideoDecoderMode=AutomaticSystemDecoder`；要吃 dav1d 须 `ForceFFmpegSoftwareDecoder`；配置在 `MediaSourceConfig.Video`，线程数显式设 `Environment.ProcessorCount`。排查看 `VIDEODEC|codec=..|mode=..`。解码策略由 `IVideoPlaybackItemFactory` 统一供给。
- 许可：FFmpegInteropX Apache-2.0，FFmpeg LGPL-2.1-or-later（动态链接、须署名）→ 见 `docs/THIRD-PARTY-NOTICES.md`。
...
- 库里 `taken_utc` 是文件系统时间，不是 EXIF 拍摄时间；EXIF `TakenAt` 只在查看器解析。
- 按类型 + 随机排序取候选可直接用既有 `QueryAsync`（`Kind` + `SortKey=Random` + 换种子）。
...
- 图库列表性能改造：P0 止损 → P1a 方形视图虚拟化 → P1b 调度器 + LRU → P2 ItemsRepeater → P3 稀疏数据源（可选）。完整方案见 `docs/图库列表性能优化方案.md`。
- 未做（勿重复评估）：非随机排序游标；机会性预取。新查询用 `EXPLAIN QUERY PLAN` 复核。
- 已推翻旧定论：① 滑动窗口「触发条件苛刻」不成立；② 埋点改为「异步化 + 默认关闭」（diag.log 是唯一线上取证手段）。
