...
- **【强制】功能是否正常一律由用户手工验证并反馈或截图**，AI 不得用自启 + 截屏 + UIA 脚本代替人工验收；自启/截图仅可用于崩溃排查与取证，不得据此宣称已修复。
...
- ItemContainerStyle 模板内 x:Bind 根是模板化控件；页面属性须经 PageProxy。**模板内 x:Bind 禁配 StaticResource Converter**（运行时 NRE、编译期 0 警告）→ 条件显隐用 VisualState；VisualState Setter 优先级高于本地绑定值。
...
## 验证手段 / 入口排查
- 验证 UI 用截屏 + UIA 枚举 ListItem（Name + 坐标）；WinUI `TextBlock` 不把 Text 暴露为 UIA Name，按钮可用 `InvokePattern`；查控件类名用 `Inspect.exe`。链路：`Start-Process` → `AppActivate(pid)` → `CopyFromScreen` 存 PNG。
- 「点了没反应」先查入口是否存在（跳转常是「按 Tag 查导航项 → 找不到静默 return」）；`git log -S '<Tag>'` 为空 = 功能从未接入。
- 验证「设置即时生效」类功能必须走真实 UI 路径，不能「改配置文件 + 重启」替代（会掩盖广播订阅缺失）。
- 加过 RID 后产物移到 `win-x64` 子目录，**旧 exe 仍留在原目录且可双击启动** → 排查「改动没生效」先确认 `(Get-Process Pixbian).Path`，再确认 ffmpeg 原生库加载（Modules 过滤 avcodec/FFmpegInteropX）。
...
- 顶栏在系统标题栏 48px 内：交互控件必须登记 Passthrough 才收得到点击；只放行按钮不放整条顶栏；可见性变化后须延一帧重算矩形。顶栏收不到 `PointerEntered/Exited`。播放态舞台恒暗 → 系统按钮前景恒取白色；顶栏 CommandBar 右留 140px 避让系统按钮。
- **控制条/覆盖层**：顶底控制条为覆盖层（不占布局行），自动隐藏用 `DispatcherQueueTimer`（3s）。**画面之上不得叠铺满的层**（打断硬件覆盖 → 每帧合成，4K 代价显著）；被完全遮挡的全窗壁纸一并隐藏；窗口根给不透明底以争取直通。
- **「播放视频 CPU 高」判别法**：先看任务管理器 GPU 页 Video Decode 占用——0% 即软解；与系统「电影和电视」同文件对照。处置三条（代价递增）：装「HEVC 视频扩展」→ `CreateFromUri` 替换 `CreateFromStorageFile` → 引入 FFmpegInteropX。教训：未证实前勿把「覆盖层打断硬件覆盖」当根因（已证伪）。
...
- 默认 `VideoDecoderMode=AutomaticSystemDecoder`（系统解码器能播就不用 FFmpeg，AV1 即此）；要吃 dav1d 须显式 `ForceFFmpegSoftwareDecoder`；配置在 `MediaSourceConfig.Video`（`VideoConfig`），线程数须显式设 `Environment.ProcessorCount`。排查看 `VIDEODEC|codec=..|mode=..` 日志。解码策略由 `IVideoPlaybackItemFactory` 统一供给，播放器页与短片页共用。
...
## 音乐库 / 索引 / 取数
- **音乐库独立于图库**：曲目落 `music_tracks`，**不进 `media_items`**（图库 `Kind=null` 会查出音频并被元数据回填反复探测）。代价：不能用 `LibraryWatcherService` → 改为设置变更时重扫 + 启动时从库恢复（机械盘全量扫盘须 `Task.Run`）；音频分类器与 `MediaFileClassifier` 分开。
