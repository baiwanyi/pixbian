...
- 已启用 `ImplicitUsings` / `Nullable` / `LangVersion 12`；CI 用 `-warnaserror`（改动须 0 警告）；缩进 4 空格；文件头 3–8 行中文模块说明。
...
## 通用工程方法论
- **性能定位顺序：先测真实数据规模 → 再测单点耗时 → 最后才改代码**。用户口述的规模必须实测（「200 万文件」实为磁盘文件数，索引仅 2.5 万条）。
- 诊断程序放**工作区外**（`C:\Temp\...`），避免污染仓库；连 WAL 库用 `SqliteOpenMode.ReadWrite` 可与应用并发读。
- SQLite 手感（2.5 万条）：`COUNT(*)` ~20ms，带排序首页 200 条 ~30ms，`OFFSET 100000` ~200ms。低于此量级先查别处。
- **后台任务让出比例比绝对时长更关键**：批次 2.5s 时节流需 ≥1.5s（让出 1/3）；并给单次运行设**批次数上限**。
- 后台常驻任务**节流 + 排他两条护栏缺一不可**；多入口触发必须收口到同一把锁（窗口层绕过 VM 直调服务是并发缺陷高发源）。排他优先 `Interlocked.CompareExchange`（持 CTS 字段会触发 CA1001，在 `-warnaserror` 下是错误）。
- 排查「卡死」先分崩溃 vs 无响应：先读 `crash.log` 时间戳，无新增崩溃即排除异常与布局循环，改查后台重活争抢。

## 工具/验证手段（可靠结论）
- `search_content` 的 `glob` **不支持 `!` 取反**（静默 0 结果）；判断忽略只能用 `git check-ignore -v <path>`。
- **查 API 是否存在一律读包内二进制，不凭记忆/文档**：WinRT 投影在 `microsoft.windows.sdk.net.ref/<ver>/winmd/`；WinUI 组件是托管程序集 `Microsoft.WinUI.dll`（`.xml` 成员索引 `T:`/`P:`/`M:` 比搜二进制精确）。winmd 不能 `Assembly.LoadFrom`（.NET 8 报 0x80131515）。
- 控件模板/主题资源默认值读 `<包>/.../Microsoft.WinUI/Themes/generic.xaml`。
- **Microsoft Learn 的 WinRT 页与中文图标表都会写错**，须以包内二进制逐项 diff；超长页面勿用 web_fetch（会截断）。查 MSBuild 属性：`dotnet msbuild xxx.csproj -getProperty:属性名`。

## WinUI 3 / WASDK 关键事实（踩过的坑）
- **2.x 的 `PkgMicrosoft_WindowsAppSDK` 已失效**，XAML 编译器路径须用 `PkgMicrosoft_WindowsAppSDK_WinUI`（否则 MarkupCompilePass1 静默失败，全项目爆 `CS0103`）。
...
- `WinUIEx` 已移除，用 `AppWindow.SetIcon(string)`；Picker 用 `Microsoft.Windows.Storage.Pickers`（构造传 `WindowId`）。
...
- **unpackaged 应用要 Win11 圆角只能靠 `MicaBackdrop`**（WASDK 2.3.6 无 `TransparentBackdrop`），Mica 仅 Win11 生效。
- **`Page.KeyboardAccelerators` 会污染页面内所有 ToolTip**（官方 by design，`PlacementMode="Hidden"` 无效）→ 只能用代码后置 KeyDown；菜单项 `KeyboardAcceleratorTextOverride` 是豁免用法。
- `ThemeShadow` + `Translation`：**z 是投影唯一输入**，z=0 几乎不可见；`Translation` 不参与布局。
...
- **ProgressRing 模板动画参与布局测量**，与同格大重排同帧会概率性 `LayoutCycleException`（启动即崩、进程仍活）。根治靠**把 loading 覆盖层放窗口层（PageHost 兄弟位）**。取证靠二分 + 连启观察（至少 8 秒 × 多次）。`x:Load` / x:Bind OneWay 只支持 Page/UserControl（非 Window），Window 层联动走代码后置 INPC 转发。
- **`MenuFlyout` 从 `Application.Current.Resources` 取出的是共享单例**，重复 `ShowAt` 抛 `E_INVALIDARG` → 可重复弹出的菜单必须工厂方法每次 `new`（「数据定义 + 工厂方法 + 每次 Opening 重建」可消掉一整类勾选同步代码）。
- **unpackaged 应用的 PRI 不索引 `<Content>` 项**，`ms-appx://` 解析不到 → 资源按 `AppContext.BaseDirectory` 磁盘路径加载；默认 Content glob 不含 `.jpg`，须显式声明。
...
- **切换 ListViewBase 的 `SelectionMode` 会重置选择**：保留选择须先抓快照、归零后再恢复。
- **符号字体码点（离屏渲染实证）**：空心文件夹 `\uED25`；`\uE8B7` 在 Fluent 是实心 FolderFill；线性星 `\uE734` / 实心星 `\uE735`；空心爱心 `\uEB51` / 实心 `\uEB52`（Symbol.Favorite 是爱心非星形）。查码点用 PowerShell + WPF `RenderTargetBitmap` 离屏渲染 PNG 目检。

## LayoutCycle 排查（判别式）
- **CPU 单核 ~100% + 业务日志 0 增长 = 布局/渲染死循环**；CPU 高但日志持续增长 = 业务慢。死循环时托管堆栈为空、`crash.log` 常不留痕迹。
- **绝对不要让「缩略图/降采样位图的尺寸」参与任何驱动布局的属性**。位图按档位量化解码，宽高比有微小偏差，一旦覆盖已有准确值就形成 **解码 → 宽高比抖动 → 重排 → 回写 → 再解码** 的环。位图尺寸只能作兜底，**优先级必须排最后**。「宽高比是相对值所以用位图无害」是错误判断。
- **覆盖层与内容网格同处一个布局容器时，同帧内既替换整页条目又折叠覆盖层会让两者测量互相失效**。撤 loading 覆盖层前须 `await` 一个渲染帧。
- **窗口级 indeterminate 动画（ProgressBar IsIndeterminate / ProgressRing）本身就是布局刺激源**：即使覆盖层已隔离到窗口层（PageHost 兄弟位），其滑块/旋转动画仍每帧搅动窗口布局 pass，与页面集合重建在同一布局根上交替失效 → LayoutCycleException。**loading 覆盖层一律用无动画静态文本**。
- **「视觉死但日志活」= 布局系统坏死而非进程死**：crash 后 XAML 停止渲染更新，但 UI 线程仍处理消息、线程池业务日志照常输出。不能凭「进程 Responding」判断界面可用。
- **时间戳对齐是循环取证核心**：crash 时刻早于业务日志 → 循环发生在数据加载前阶段，可直接排除集合重建期的嫌疑环节。自证日志（PANEL 计数）无循环特征同样关键——**证明面板无辜**，嫌疑上移窗口级。
...
- 透出背景：`NavigationView{Default,Expanded,Top}PaneBackground` 与 `NavigationViewContentBackground` 全改 `Transparent`，NavigationView 自身 `Background` 也要透明。**Pane 展开态走 `NavigationViewExpandedPaneBackground`**；死键：`NavigationViewPaneBackground`/`ContentBackground`/`Background`。
...
- **Pane 与内容区之间的竖线来自 `ContentGrid` 的 `BorderThickness.Left=1`**，非 Pane 自身 Border。左栏圆角来自模板 `RightCornerRadiusFilterConverter`，元素级覆盖 `OverlayCornerRadius` 无效。
...
- 异步加载治理：**信号量只控并发数，不控「该不该做」**；可见性判定用 `ContainerFromItem(item) is null`；滚走取消 + 滚回重触发。**删除「整页提交」兜底是高危操作**。
...
## 索引与元数据回填
- 索引分**两阶段**：扫描只写文件属性（秒级，界面立即可浏览）；宽高与时长由后台回填服务分批补齐。
...
- 历史遗留 `Diagnostics.Log`：`DBLTAP`、`VIEWER`、`THUMB`，以及基线测量新增的 `LOAD`/`STAT`/`PROBE`/`THUMBWAIT`/`LOADTOTAL`。
