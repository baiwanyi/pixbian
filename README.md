...
| 备份与同步 | 用户数据导出 / 导入（单个 JSON 数据包，跨机迁移，导入支持路径前缀重映射）；同步到 OneDrive（手动 / 每天 / 每周 / 每月，启动时检查并补做错过的周期）；索引库整库快照与还原（`VACUUM INTO`，同机灾备） |
...
| 用户数据备份 | 导出位置由用户在保存对话框中指定；OneDrive 同步写入 `<OneDrive>\Pixbian\`（含 `history\` 历史副本） |
...
四套 xunit 测试，共 **390 个用例**（Core 260 / WebServer 65 / Imaging 14 / UI 51），可在无 UI、无网络的 CI 环境通过：

- `Pixbian.Core.Tests`：`PathGuard`、媒体文件分类器、索引服务、查询构建、分类规则引擎、SQLite 仓储、元数据回填、缩略图磁盘缓存、日志脱敏；用户数据备份（往返一致 / 重复导入幂等 / 路径重映射与目录边界 / 坏文件与超限文件拒绝 / 导出不含凭据）、OneDrive 同步（周期判定）与整库快照（导出 / 还原 / 拒绝非法快照）
...
- **清理入口目录会在启动时保留**：`%LOCALAPPDATA%\Pixbian\*.bak-*`（整库还原产生的旧库副本）不会自动清理。
