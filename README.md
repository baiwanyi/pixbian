...
| 查看器 | 缩放 0.1×–8×、旋转、幻灯片播放、EXIF 信息面板、按 EXIF 方向自动摆正；放大后按住拖动平移（以光标为锚缩放、边界钳制）；显示旋转（界面）与落盘旋转（服务层）分离，落盘一律写入新文件，当前仅服务层可用 |
| 视频 | 系统解码器播放，倍速 0.25×–4×、音量 / 静音、进度拖动；元数据（时长 / 分辨率 / 编码 / 码率 / 音轨） |
| 分类 | 自定义分类 + 正则规则（匹配文件名 / 完整路径 / 扩展名），规则启停、优先级排序、批量重匹配 |
| 收藏 | 收藏 / 取消收藏；自建收藏分组（分组与条目多对多），「未分组」为查询语义而非一条记录 |
| 索引 | 添加 / 移除扫描源；全量扫描与前缀对账自动清理失效条目；尺寸与时长由后台任务分批回填；扫描源目录不存在时左栏子项灰显禁用 |
| 缺失标记 | 文件被移动或删除后，条目显示占位图标 + 文件名 + 完整路径（文本按格子尺寸自适应），并提供「从图库移除记录」清理入口（仅删索引、不动文件） |
| 备份与同步 | 用户数据导出 / 导入（单个 JSON 数据包，跨机迁移，导入支持路径前缀重映射）；同步到 OneDrive 的「应用 / Pixbian」（成对写出数据包与整库快照，手动 / 每天 / 每周 / 每月，启动时检查并补做错过的周期）；索引库整库快照与还原（`VACUUM INTO`，同机灾备）；文件名统一为 `backup-pixbian-userdata|index-<时间戳>.json|db` |
| 局域网 | 内置 HTTP 服务，浏览器浏览与流式播放；密码保护、只读、IP 限流；可指定监听网卡收敛暴露面（**明文 HTTP，仅限可信专用网络**，见[安全前提](docs/局域网共享安全前提.md)） |
...
**数据备份。** 用户数据（收藏与评分、收藏分组、分类与规则、扫描源、音乐目录）导出为单个 JSON 数据包：条目一律以 `media_items.path` 为业务键、分类与分组按名称互引——自增主键在换机或重建索引后必然变化，不能作跨库标识。导入为「合并、导入文件为准」：收藏取并集、评分只升不降、分组成员与扫描源 `INSERT OR IGNORE` 保证幂等，整体落在**单事务**内；入口设四道闸门（格式与版本校验、64 MB 大小上限、规则正则经 `ValidatePattern` 拦截非法与超长模式、路径前缀必须落在目录边界以免 `D:\Lib` 误命中 `D:\Library`）。同步到 OneDrive 写入 `<OneDrive>\Apps\Pixbian\`（即资源管理器里的「应用 / Pixbian」）：每次同步**成对**写出 `backup-pixbian-userdata-<yyyyMMdd-HHmmss>.json`（数据包）与 `backup-pixbian-index-<yyyyMMdd-HHmmss>.db`（整库快照），两份文件共用同一时间戳，故按时间戳**成组保留最近 5 组**、不会只留半个；因应用非常驻，「每天 / 每周 / 每月」只能靠**启动时检查 + 补齐错过周期**实现。整库快照走 `VACUUM INTO`（一致性在线快照，避免复制到写了一半的 WAL 半成品），还原时保留 `.bak` 副本、清理旧库的 `-wal` / `-shm`，完成后需重启应用；`.bak` 副本在启动时按「保留最近 3 份且不超过 30 天」自动清理。
...
| 用户数据备份 | 导出位置由用户在保存对话框中指定（默认名 `backup-pixbian-userdata-<时间戳>.json`，快照为 `backup-pixbian-index-<时间戳>.db`）；OneDrive 同步写入 OneDrive「应用」目录下的 `Pixbian\`，按时间戳成组保留最近 5 组 |
...
局域网服务**默认关闭**，开启时建议设置密码，并仅在可信的「专用」网络下使用。
...
四套 xunit 测试，共 **406 个用例**（Core 276 / WebServer 65 / Imaging 14 / UI 51），可在无 UI、无网络的 CI 环境通过：

- `Pixbian.Core.Tests`：`PathGuard`、媒体文件分类器、索引服务、查询构建、分类规则引擎、SQLite 仓储、元数据回填、缩略图磁盘缓存、日志脱敏；用户数据备份（往返一致 / 重复导入幂等 / 路径重映射与目录边界 / 坏文件与超限文件拒绝 / 导出不含凭据）、备份文件名规则（构造与反解）、OneDrive 同步（周期判定 / 成对落盘 / 成组清理 / 拒绝相对路径）与整库快照（导出 / 还原 / 拒绝非法快照）
- `Pixbian.WebServer.Tests`：鉴权与限流、HTTP 解析、路由与 Range、连接治理与安全防护
- `Pixbian.Imaging.Tests`：图像编辑服务
- `Pixbian.UI.Tests`：缩略图调度器、等高视图选择服务、图库与条目视图模型（含分页、删除链路与缩略图四态状态机）

---

## 当前状态与已知未完成项

已实现并可用：索引与浏览、查看器、视频播放、正则分类、收藏与分组、数据备份与 OneDrive 同步、局域网访问、设置与主题。
...
- **备份同步依赖启动时检查**：应用非常驻，周期同步只在启动时补做；若需要「应用未运行也同步」，须注册系统计划任务并提供独立的命令行入口（当前未规划）。
